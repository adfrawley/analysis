...
void analyze_evaluator_tracks(){
  ...
  // for Pythia events, we need to exclude anything not detectable! Choose charged pions and protons
    ...
    TCut good_track_cut = Form("gtrackID>=0 && gprimary == 1 && abs(gvz) < 10 && abs(geta) < 1  && (abs(gflavor) == 211 || abs(gflavor) == 2212)  && quality < 10");   
  ...
  TCanvas *ctrgvt = new TCanvas("ctrgvt","track gvt signal");
  TH1D *htrgvt = new TH1D("htrgvt","track gvt signal",44000,-20000.0, 24000.0);
  ...
  TH1D *hgvt_pu = new TH1D("hgvt_pu","gvt pileup vertices",44000,-20000.0,24000.0);
