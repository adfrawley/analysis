...
//----------------
// To aggregate the output root files in quarkonia_analyze_out from here:
// modify hadd_bash_script to have the correct number of files
//  bash
//  source hadd_quarkonia_bash_script
//  exit
//----------------

void quarkonia_reconstruction_embedded_pp_condor(int proc = 0, int n_list = 5)
{
  /*
  ...
  */
  ...
  for(int i=nstart;i<n_list+nstart;i++)
      ...
      if(nrecormass > nups_requested)
	{
      ...
      sprintf(name,"/sphenix/user/frawley/new_macros_april27/macros/detectors/sPHENIX/no_elecID_pp_pileup_eval_out/g4svtx_eval_%i.root",ifile);
...
#include "ntuple_variables.C"
  ...
  */
  ...
  sprintf(fname,"analyze_quarkonia_out/ups_embed_out_%i.root",proc);
